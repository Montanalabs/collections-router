...
Untrusted input is an account; it can only ever become one of a fixed set of decisions over a
...
- **Untrusted input:** `fetch<web>` — an account
- **Closed type:** `type Decision = RouteColl(CollStage) | HoldColl` where `CollStage` is closed
- **Sink / capability:** `grant routeCollection` → `privileged { routeCollection(d) }`
- **Consequence axes:** Trust

## Run the demo

```sh
examples/apps/collections-router/demo.sh
```
...
- `collections-router_safe.eide` — the correct design.
- `collections-router_unsafe.eide` — the tempting-but-wrong version (the negative example a model must learn to reject).
